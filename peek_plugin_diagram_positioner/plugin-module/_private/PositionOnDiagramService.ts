...
export class PositionOnDiagramService extends NgLifeCycleEvents {
    ...
    private async addLocateOnActions(
    ...
    private async loadPositions(
        ...
        const locations: DispKeyLocation[] =
            await this.positionService.locationsForKey(modelSetKey, dispKey);

        for (const location of locations) {
            // Ignore positioning on the same coord set
            if (location.coordSetKey == coordSetKey) continue;

            const name =
                this.MODEL_SETS[modelSetKey].name +
                " - " +
